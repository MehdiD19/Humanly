    ...
    try {
      ...
      if (!response.ok) {
        const errorData = await response
          .json()
          .catch(() => ({ error: "Failed to get LiveKit token" }));
        throw new Error(
          errorData.error || "Failed to get LiveKit token from backend"
        );
      }

      const data = await response.json();

      // Use the URL from the backend response (it knows the correct LiveKit URL)
      // Fallback to environment variable if not provided
      return {
        token: data.token,
        url: data.url || LIVEKIT_API_URL,
      };
    } catch (error) {
